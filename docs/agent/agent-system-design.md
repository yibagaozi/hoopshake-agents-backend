...
**作者须知(标注段格式)**:小节内的标注段(如"技术动作要点:""扶球位置:")**加粗与否是纯装饰**——`MarkdownDocumentReader` 会剥掉 `**`,不进 embedding、不影响切分。真正起作用的是两样:①**标签词本身**(是语义信号,别省);②**每段前后留空行**(空行才是超限兜底二次切的边界)。加粗建议保留(源文件更易读、零副作用),但**全库统一一种写法**即可。**切勿把标注段升成 `#####` 子标题**——那会把"整节一块"碎成多个过小块。
