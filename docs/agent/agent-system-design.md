...
**作者须知(标注段格式)**:小节内的标注段(如"技术动作要点:""扶球位置:")**加粗与否是纯装饰**——`MarkdownDocumentReader` 会剥掉 `**`,不进 embedding、不影响切分。真正有语义价值的是**标签词本身**(别省)。加粗建议保留(源文件更易读、零副作用),但**全库统一一种写法**即可。**切勿把标注段升成 `#####` 子标题**——那会把"整节一块"碎成多个过小块。

关于空行,澄清两点(避免误读):
- chunk 边界只在**标题层级**(`##`/`####`)形成;一个"优先级块 / 技术小节"内部**无论有多少空行都是同一个 chunk**(块 <450 token,不触发二次切)。所以"动作要领:"和它的要点之间那个空行**对切分无影响**,留着只为列表解析干净。
- 我前文说"空行是兜底边界",仅指**顶层标注段之间**的空行,且**只有当某块超过 `chunkMaxTokens` 才用到**——你的语料都远小于 450,现在一个都用不上。真触发时,二次切应把"标签行 + 其紧跟的列表"当一个整体,不在两者间断开。
- **不要在正文里塞文档结构说明**(如"本文按优先级排序:错误表现→…")当作知识——那不是篮球知识,对召回是噪声;需要的话放 frontmatter `description`(不进 embedding)。
