...
public enum EdgeErrorCode implements ErrorCodeSpec {
    ...
    /** 本课标定产物缺失:未标定就跑 CV,三角化不可用、角度不可信 */
    CALIBRATION_MISSING(40917, HttpStatus.CONFLICT, "本课标定产物不存在,请先完成球场标定"),
    /** 同一时刻只允许一个标定任务 */
    CALIBRATION_BUSY(40918, HttpStatus.CONFLICT, "已有标定任务在进行中"),
    ...
    /** 标定编排未启用或命令未配置 */
    CALIBRATION_UNAVAILABLE(50333, HttpStatus.SERVICE_UNAVAILABLE, "球场标定编排不可用"),
    /** ffmpeg 家族:进程本身不可用 */
